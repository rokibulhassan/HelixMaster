...
function anchorNavigation(){
	...
	jQuery('.side-menu').each(function(){
		...
		var navLinks = panel.find('a').add(jQuery('#nav a'));
		navLinks.parent().removeClass(activeClass);
		navLinks.each(function(){
			var navLink = jQuery(this);
			var anchorBlock = jQuery(navLink.attr('href'));
			setActive();
			function setActive(){
				if (anchorBlock.length) {
					if (win.scrollTop() >= anchorBlock.offset().top && win.scrollTop() < anchorBlock.offset().top + anchorBlock.outerHeight()) 
						navLink.parent().addClass(activeClass);
					else 
				...
				if(isTouchDevice) {
					if(!isWinPhoneDevice) {
						navLink.bind('click', function(e){
							e.preventDefault();
							jQuery('html, body').stop().animate({scrollTop: anchorBlock.offset().top}, animSpeed);
						});
					}
				}
				else {
			...
			win.bind('scroll resize', setActive);
		});
	})
}
...
function initSameHeight() {
	jQuery('div.item-block').sameHeight({
		elements: 'div.box',
		flexible: true,
		multiLine: true
	});
